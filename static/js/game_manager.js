...
GameManager.prototype.startAI = function(){
    ...
    var self = this;
    setTimeout(function(){
        if (!self.isGameTerminated() && self.autoActive) {
            self.aiMove(); // Excludes root level branches
            self.startAI();
        }
    }, 300);
}
    ...
    for (var x = 0; x < 4; x++) {
        for (var y = 0; y < 4; y++) {
            if (this.grid.cellOccupied({x: x, y: y})) {
                _grid[y][x] = this.grid.cellContent({ x: x, y: y }).value;
            } else {
                _grid[y][x] = 0;
            }
        }
...
//    console.log("grid:", _grid);
    ...
    (async () => {
    try {
        best = await bestMove(_grid);
        this.move(best);
//        console.log('Data received:', best);
    } catch (error) {
